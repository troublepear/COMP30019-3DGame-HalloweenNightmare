			...
			if (ParticleSystems.Count > 0) {
				for (int i = 0; i < ParticleSystems.Count; i ++) {
					...
					if (ps != null) {
						...
						if (!mainSystemScript.CheckExistingFile (gameObject)) {								
							...
							listOriginalSettings = mainSystemScript.LoadVFX (gameObject);
				...
				if (!mainSystemScript.CheckExistingFile (gameObject)) {		
					mainSystemScript.SaveVFX (gameObject, psOriginalSettingsList);
                }
#if UNITY_2018_3_OR_NEWER
                else
                {
                    mainSystemScript.SaveNestedPrefab(gameObject);
                }
#endif
            }
			else
